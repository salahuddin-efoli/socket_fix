...
export const loader = async ({ request }) => {
    ...
    if(currentAgent.role != "ADMIN" && !currentAgent.permissions.includes("FAQ_CRT")) {
...
export const action = async ({ request }) => {
    ...
    if (target == "create-faq") {
        ...
        const serial = faq.serial ? parseInt(faq.serial) : "";
        const title = faq.title || "";
        const description = faq.description || "";
        const status = faq.status || "";

        /**
         * * Insert a new FAQ with the appropriate serial number
         * TODO: Initialize the new serial number to 1 by default
         * TODO: If a specific serial number is provided:
         *        - Shift existing FAQs down by one position to make room for the new FAQ
         *        - Update the serial numbers of FAQs that are greater than or equal to the provided serial
         *        - Set the new serial number to the provided value
         * TODO: If no specific serial number is provided:
         *        - Find the current maximum serial number among existing FAQs
         *        - Set the new serial number to maxSerial + 1, or 1 if no FAQs exist
         * TODO: Create the new FAQ with the determined serial number and provided title, description, and status
         */
        try {
            let newSerial = 1;
            if (serial != "") {
                ...
                const maxSerial = await prisma.faqs.aggregate({
                    _max: {
                        serial: true,
                    },
                });
...
export default function FaqCreate() {
    ...
    const submitForm = async () => {
        ...
        if(validated) {
            submit({ target: "create-faq", data: JSON.stringify(formState) }, { method: "POST" });
    ...
    useEffect(() => {
        if (actionData) {
            if (actionData.target == "success") {
                setFormState({ serial: "", title: "", description: "", status: "ACTIVE" });
                setFormError({ serial: "", title: "", description: "", status: "" })
            }
        }
    }, [actionData])
